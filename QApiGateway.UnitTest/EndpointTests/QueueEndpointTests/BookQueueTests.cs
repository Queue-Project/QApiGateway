...
public class BookQueueTests
{
    ...
    [Fact]
    public async Task BookQueue_Should_Return_BookedQueue_When_Authorized()
    {
        // Arrange
        var request = new QueueCreateRequest
        {
            CompanyId = 1,
            BranchId = 1,
            ServiceId = 1,
        ...
        // Act
        var result = await QueueEndpointHandlers.BookQueueHandler(
            client,
    ...
    [Fact]
    public async Task BookQueue_Should_Return_Error_When_Company_Not_Found()
    {
        // Arrange
        var request = new QueueCreateRequest
        {
        ...
        var expectedResponse = "{error: Company not found}";
        var token = "Bearer test-token";
